...
export default async function OpportunitiesPage({ searchParams }: { searchParams: Promise<{ filter?: string; q?: string; page?: string }> }) {
  ...
  const result = await getOpportunities({ page, pageSize: 20, filter: params.filter, query: params.q });
  return <><PageHeader eyebrow="PIPELINE" title="Todas as oportunidades." description="Acompanhe publicações normalizadas, abra a análise e transforme uma descoberta em participação." />
    <div className="content-stack">{!result.configured && <SetupNotice />}{result.error && <DataError message={result.error} />}
      ...
      <div className="section-heading"><div><h2>{result.count} registros</h2><p>Ordenados pelo encerramento mais próximo.</p></div></div>
      {result.data.length ? <div className="opportunity-list">{result.data.map((opportunity) => <OpportunityCard key={opportunity.id} opportunity={opportunity} />)}</div> : <EmptyState title="Nenhuma oportunidade encontrada" description="Ajuste os filtros ou sincronize o PNCP no Radar." href="/radar" action="Abrir radar" />}
