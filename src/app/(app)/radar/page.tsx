...
export default async function RadarPage({ searchParams }: { searchParams: Promise<{ filter?: string; q?: string; page?: string }> }) {
  ...
  return <><PageHeader eyebrow="RADAR GOV" title="Pequenas oportunidades. Boa aderência. Pouco caixa preso." description="Consulta fontes oficiais, deduplica oportunidades e ordena para a realidade da SKULL Tecnologia. PNCP e Compras.gov já estão ativos." action={<SyncButton enabled={session.role === "skull_admin"} />} />
    <div className="content-stack">
      ...
      {result.data.length ? <div className="opportunity-list">{result.data.map((opportunity) => <OpportunityCard key={opportunity.id} opportunity={opportunity} />)}</div> : <EmptyState title="O radar está limpo" description="Não há dados para este filtro. Execute uma sincronização como SKULL Admin para consultar as fontes oficiais." />}
